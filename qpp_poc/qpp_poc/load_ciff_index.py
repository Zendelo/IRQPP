...
def parse_messages(buffer, n, message_type):
...
def parse_index_file(index_file):
    ...
    with open(index_file, 'rb') as fp:
        buf = fp.read()
        n = 0
        cur_n, header = parse_messages(buf, n, ciff.Header)
        num_postings_lists = header.num_postings_lists
        for _ in range(num_postings_lists):
            n, _posting_list = parse_messages(buf, cur_n, ciff.PostingsList)
            terms_dict[_posting_list.term] = cur_n
            cur_n = n
        num_doc_records = header.num_docs
        for _ in range(num_doc_records):
            n, _doc_record = parse_messages(buf, cur_n, ciff.DocRecord)
            doc_records[_doc_record.docid] = cur_n
            cur_n = n
...
class Index:
    ...
    def _read_index_line(self, n):
        _, posting_list = parse_messages(self.file_buf, n, ciff.PostingsList)
        return posting_list
