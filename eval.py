...
def testing(predictor_exe, parameters_xml, test_param):
    run('mkdir -v tmp-testing', shell=True, check=True)
    ...
    for i in [5, 10, 25, 50, 100, 250, 500, 1000]:
        ...
        run('{} {} {}{} > tmp-testing/testing-{3}'.format(predictor_exe, parameters_xml, test_param, i), shell=True,
            check=True)
        run('python correlation.py baseline/QLmap1000 tmp-testing/testing-{}'.format(i), shell=True, check=True)
...
def main(args):
    ...
    testing(predictor_exe, parameters_xml, test_parameter)
