...
/*This is the function that prints the stemmed query*/
  void _stemQuery( std::stringstream& output, const std::string& query,
    ...
    {
      _results.clear();
      LEMUR_RETHROW(e, "Query stemming function crashed with Exception");
    }
  }
  ...
  UINT64 work() {
    ...
    try {
      if (_parameters.exists("baseline") && ((query->text.find("#") != std::string::npos) || (query->text.find(".") != std::string::npos)) ) {
	   ...
	   _stemQuery( output, query->text, query->qType, query->number);
    } catch( lemur::api::Exception& e ) {
